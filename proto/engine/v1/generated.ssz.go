...
func (b *BlindedBlobsBundle) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	for ii := 0; ii < len(b.BlobRoots); ii++ {
		if size := len(b.BlobRoots[ii]); size != 32 {
			err = ssz.ErrBytesLengthFn("--.BlobRoots[ii]", size, 32)
			return
		}
...
func (b *BlindedBlobsBundle) UnmarshalSSZ(buf []byte) error {
	...
	// Field (2) 'BlobRoots'
	{
		...
		b.BlobRoots = make([][]byte, num)
		for ii := 0; ii < num; ii++ {
			if cap(b.BlobRoots[ii]) == 0 {
				b.BlobRoots[ii] = make([]byte, 0, len(buf[ii*32:(ii+1)*32]))
			}
			b.BlobRoots[ii] = append(b.BlobRoots[ii], buf[ii*32:(ii+1)*32]...)
		}
	}
	return err
}
